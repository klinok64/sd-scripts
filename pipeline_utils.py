...
def is_safetensors_compatible(filenames, variant=None, passed_components=None) -> bool:
    ...
    for filename in pt_filenames:
        ...
        if expected_sf_filename not in sf_filenames:
            print(f"{expected_sf_filename} not found")
            return False
...
def _get_pipeline_class(
    ...
    if load_connected_pipeline:
        ...
        if connected_pipeline_cls is not None:
            ...
            print(f"{pipeline_cls.__name__} has no connected pipeline class. Loading {pipeline_cls.__name__}.")
...
def load_sub_model(
    ...
    if os.path.isdir(os.path.join(cached_folder, name)):
        print(f"\n\n2Checkpoint2. Load_method_name: {load_method_name}\n\n:Load_method : {load_method}\n\n")
        ...
        **kwargs,
        ...
        for pipeline_component_name in model_index_dict.keys():
            ...
            if save_method_name is None:
                print(f"self.{pipeline_component_name}={sub_model} of type {type(sub_model)} cannot be saved.")
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        for name, (library_name, class_name) in init_dict.items():
            ...
            if name in passed_class_obj:
                ...
                # load sub model
                print(f"\n\nCheckpoint5. Loading {name} as {class_name} from `{name}` subfolder of {pretrained_model_name_or_path}.\n\n")
    ...
    def download(cls, pretrained_model_name, **kwargs) -> Union[str, os.PathLike]:
        ...
        if not local_files_only:
            try:
                ...
                print(f"Couldn't connect to the Hub: {e}.\nWill try to load from local cache.")
